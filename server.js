...
const server = http.createServer(async (req, res) => {
    ...
    if (pathname === "/api/optimized-allocation") {
        try {
            const allRoutes = loadRoutes();
            const busState = loadBusState();

            // Step 1: Compute each route's GTFS distance
            const routesWithDistance = allRoutes.map((route) => {
                const distResult = calculateRouteDistance(route.busNumber);
                const distanceKm = distResult.ok ? distResult.distanceKm : 0;
                return { ...route, distanceKm };
            });

            // Step 2: Sort routes by distance DESCENDING (longest first)
            const sortedRoutes = routesWithDistance
                .slice()
                .sort((a, b) => b.distanceKm - a.distanceKm);

            // Step 3: For each route, get its bus state (SOC + condition)
            const busEntries = allRoutes.map((route) => {
                const state = busState[route.routeId] || busState[route.busNumber] || {};
                const soc = Number.isFinite(Number(state.soc)) ? Number(state.soc) : 100;
                const condition = state.condition || "Good";
                return { routeId: route.routeId, busNumber: route.busNumber, soc, condition, originalRoute: route };
            });

            // Step 4: Sort buses by SOC DESCENDING (highest charge first)
            const sortedBuses = busEntries
                .slice()
                .sort((a, b) => b.soc - a.soc);

            // Step 5: Pair them 1-to-1 — highest SOC bus → longest distance route
            const allocations = [];
            const pairCount = Math.min(sortedRoutes.length, sortedBuses.length);

            for (let i = 0; i < pairCount; i++) {
                const bus = sortedBuses[i];
                const route = sortedRoutes[i];
                const usableSoc = Math.max(0, bus.soc - 10);
                const estimatedRangeKm = Math.min(150, Math.round(usableSoc * 1.42));
                const routeDistanceKm = route.distanceKm;

                let status = "allocated";
                let blockReason = null;

                // Block if estimated range < route distance
                if (estimatedRangeKm < routeDistanceKm) {
                    status = "blocked";
                    blockReason = "Insufficient range";
                }

                // Block if condition is Not Good
                ...
                allocations.push({
                    ...
                    originalRoute: bus.originalRoute.routeName,
                    assignedRoute: route.routeName,
                    assignedRouteId: route.routeId,
                    assignedBusNumber: route.busNumber,
                    origin: route.origin,
            ...
            const blocked = allocations.filter((a) => a.status === "blocked").length;

            sendJson(res, {
                ok: true,
                allocations,
                summary: {
                    totalBuses: allRoutes.length,
                    allocated,
                    blocked
                }
            });
        } catch (error) {
            sendJson(res, { ok: false, error: "Failed to compute optimized allocation", details: error.message }, 500);
        }
        return;
    }
