...
function calculateRouteDistanceKm(routeId) {
    if (!gtfsBundle || !routeId) {
        return null;
    }
    ...
    const directionZeroTrip = routeTrips.find((trip) => Number(trip.directionId) === 0);
    const shapeId = (directionZeroTrip || routeTrips[0]).shapeId;
    const shapePoints = gtfsBundle.shapes
        .filter((point) => String(point.shapeId) === String(shapeId))
        .slice()
    ...
    for (let index = 1; index < shapePoints.length; index += 1) {
        ...
        const previousLat = Number(previous.shapePtLat);
        const previousLon = Number(previous.shapePtLon);
        const currentLat = Number(current.shapePtLat);
        const currentLon = Number(current.shapePtLon);

        if (![previousLat, previousLon, currentLat, currentLon].every(Number.isFinite)) {
            continue;
        }

        totalDistanceKm += haversineKm(previousLat, previousLon, currentLat, currentLon);
    }
...
function buildRouteStopPoints(routeId) {
    if (!gtfsBundle) {
        return [];
    }

    const routeTrips = gtfsBundle.trips.filter((trip) => String(trip.routeId) === String(routeId));
    const trip = routeTrips.find((entry) => Number(entry.directionId) === 0) || routeTrips[0];
    if (!trip) {
        return [];
    }

    return gtfsBundle.stopTimes
        .filter((stopTime) => String(stopTime.tripId) === String(trip.tripId))
        .slice()
        .sort((left, right) => Number(left.stopSequence) - Number(right.stopSequence))
        .map((stopTime) => gtfsBundle.stopsById.get(String(stopTime.stopId)))
        .filter(Boolean)
        .map((stop) => ({
            lat: Number(stop.stopLat),
            lng: Number(stop.stopLon),
            name: stop.stopName || stop.stopId
        }))
        .filter((point) => Number.isFinite(point.lat) && Number.isFinite(point.lng));
}

function buildRouteEndpointPoints(route) {
    if (!gtfsBundle || !route) {
        return [];
    }

    const findStop = (name) => gtfsBundle.stops.find((stop) => {
        const stopName = normalize(stop.stopName || stop.stopDesc || "");
        return stopName === normalize(name) || stopName.includes(normalize(name));
    });
    const origin = findStop(route.origin);
    const destination = findStop(route.destination);
...
async function drawRoutePathForBus(bus) {
    ...
    const bundle = await ensureGtfsBundle();
    const route = bundle.routes.find((entry) => String(entry.routeId) === String(bus.routeId));
    const routeTrips = route
        ? bundle.trips.filter((trip) => String(trip.routeId) === String(route.routeId))
        : [];
    const trip = routeTrips.find((entry) => Number(entry.directionId) === 0) || routeTrips[0];
    ...
    let routePath = bundle.shapes
        .filter((point) => String(point.shapeId) === String(trip.shapeId))
        .slice()
        .sort((left, right) => Number(left.shapePtSequence) - Number(right.shapePtSequence))
        .map((point) => ({ lat: Number(point.shapePtLat), lng: Number(point.shapePtLon) }))
        .filter((point) => Number.isFinite(point.lat) && Number.isFinite(point.lng));

    if (routePath.length < 2) {
        routePath = buildRouteStopPoints(bus.routeId);
    }

    if (routePath.length < 2) {
        routePath = buildRouteEndpointPoints(route);
    }
    ...
    const routeStops = buildRouteStopNames(bus.routeId);
    clearMapOverlays();
    routePolyline = new window.google.maps.Polyline({
        path: routePath,
        geodesic: true,
    ...
    const bounds = new window.google.maps.LatLngBounds();
    routePath.forEach((point) => bounds.extend(point));
    const endpointMarkers = [routePath[0], routePath[routePath.length - 1]];
    endpointMarkers.forEach((position, index) => {
        const marker = new window.google.maps.Marker({
            position,
            map: googleMap,
            label: index === 0 ? "A" : "B",
            title: routePath[index].name || (index === 0 ? bus.origin : bus.destination)
        });
        routeStopMarkers.push(marker);
        bounds.extend(position);
    });
...
async function renderMap(bus) {
    ...
    const routeStops = bus.stops.length ? bus.stops : [bus.origin, bus.destination];
    const fallbackStops = routeStops.map((name, index) => `<li>${index + 1}. ${name}</li>`).join("");
